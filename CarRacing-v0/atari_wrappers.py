...
class StopOnMinReward(gym.Wrapper):
    def __init__(self, env, min_score):
        """Done after total reward hits min_score"""
        gym.Wrapper.__init__(self, env)
    ...
    def step(self, action):
        ...
        done = done or self.total_score <= self.min_score
...
def wrap_deepmind(env, random_starts=True, dim=80):
    ...
    """
    env = NoopResetEnv(env, noop_max=50, random_starts=random_starts)
